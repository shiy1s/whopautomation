...
SUBMISSION_PATH = "state/phase13-content-rewards-submissions.json"
...
def write_json(path, payload, sha, message):
    ...
    return gh_api(
        "contents/" + urllib.parse.quote(path), "PUT", body
    )["content"]["sha"]
...
def validate_inputs():
    ...
    if PLATFORM not in {"youtube", "instagram"}:
        die("Only youtube and instagram are enabled in Phase 13; TikTok remains deferred")
...
def validate_url(url, remote):
    if PLATFORM == "youtube":
        ...
        match = INSTAGRAM_RE.fullmatch(url)
        if not match:
...
def main():
    ...
    expected = expected_url(publication.get("remote", {}))
    url = POST_URL or expected
    ...
    duplicate = any(
        ...
        and item.get("status") in {
            "prepared", "submitted", "approved", "pending", "rejected"
        }
        for item in state.get("submissions", [])
    )
    if duplicate:
        die(
            "Duplicate submission candidate already exists for this campaign, "
            "clip, platform and URL"
        )
    ...
    record = {
        ...
        "whopSubmission": {
            ...
            "reason": (
                "No supported public Content Rewards submission API was verified; "
    ...
    print(json.dumps({
        "phase13": "submission_candidate_ready",
        "status": "prepared",
        "campaignId": CAMPAIGN_ID,
        "campaignName": CAMPAIGN_NAME,
        "platform": PLATFORM,
